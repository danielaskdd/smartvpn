...
__network_parse_ifstatus()
{
	...
	for __tmp in "" "_inactive"; do
		...
		for __field in "up" "l3_device" "device" "autostart" "pending"; do
...
{
	local __autostart
	__network_device __autostart  "$1" autostart && [ $__autostart -eq 1 ]
}

network_is_pending()
{
	local __pending
	__network_device __pending  "$1" pending && [ $__pending -eq 1 ]
}
